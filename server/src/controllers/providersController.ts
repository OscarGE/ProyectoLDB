...
class ProvidersController {
  ...
  public async register(req: Request, res: Response): Promise<void> {
    ...
    await pool.query('INSERT INTO providers(name,description) VALUES ("'+req.body.name+'", "'+req.body.description+'");',async function(err, result, fields){
      if (err) throw err;
      if(result.affectedRows==1){
        await pool.query('SELECT id FROM providers WHERE name = "'+req.body.name+'";',async function(err, result, fields){
          if (err) throw err;
  ...
  public async isExistProvider(req: Request, res: Response): Promise<any>{
    const { name } = req.params;
      await pool.query('SELECT * FROM providers WHERE name = ?', [name],function(err, result, fields) {
        if (err) throw err;
        if(result.length>0){
